...
from dados import (
        link_foto, matricula, nomecompleto, cpf, datanascimento, sexo, tipo_sanguineo,
        estado_civil, datacasamento, profissao, naturalidade, nacionalidade, rua, complemento,
        bairro, municipio, estado, cep, tel_residencial, tel_celular, email, nome_pai, pai_membro,
        nome_mae, mae_membro, nome_conjuge, datanascimento_conjuge, conjuge_membro,
        cargo_atual, databatismo, igrejabatismo, entradaPIB, formaentrada, nome_filho1, 
        datanascimento_filho1, filho1_membro, nome_filho2, datanascimento_filho2, filho2_membro,
        nome_filho3, datanascimento_filho3, filho3_membro, nome_filho4, datanascimento_filho4, 
        filho4_membro, nome_filho5, datanascimento_filho5, filho5_membro
)
...
def main(pagina):
    ...
    lista_nomes = ft.TextField(
        label="Nome Completo",
        on_change=lambda e: atualizar_sugestoes(e, lista_nomes, sugestoes, nomes_completos, pagina)
    )
    ...
    # Função para upload direto para o Google Drive
    def upload_to_drive(file_stream, file_name):
        ...
        # Carrega o arquivo diretamente para o Google Drive
        file_metadata = {'name': file_name}
        media = MediaIoBaseUpload(file_stream, mimetype='image/jpeg')
        file = service.files().create(body=file_metadata, media_body=media, fields='id').execute()

        # Configura as permissões para compartilhamento público
        file_id = file.get('id')
    ...
    def on_file_picked(result):
        if result.files:
            ...
            with open(file_path, 'rb') as file_stream:
                link = upload_to_drive(file_stream, file_name)
    ...
    janela_recadastro = ft.AlertDialog(
        ...
        content=ft.Column(
            controls=[
                ft.Row(controls=[
                    ft.Column(controls=[
                        B_foto, matricula, nomecompleto, cpf, datanascimento, sexo, tipo_sanguineo,
                        estado_civil, datacasamento, profissao, naturalidade, nacionalidade, rua, complemento,
                        bairro, municipio, estado, cep, tel_residencial, tel_celular, email, nome_pai, pai_membro,
                        nome_mae, mae_membro, nome_conjuge, datanascimento_conjuge, conjuge_membro, cargo_atual,
                        databatismo, igrejabatismo, entradaPIB, formaentrada, nome_filho1, datanascimento_filho1, 
                        filho1_membro, nome_filho2, datanascimento_filho2, filho2_membro, nome_filho3, 
                        datanascimento_filho3, filho3_membro, nome_filho4, datanascimento_filho4, filho4_membro, 
            ...
            scroll=True,
            width=300  # Largura da janela (tamanho ajustado)
        )
    )
