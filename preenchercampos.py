...
from dados import (matricula, nomecompleto, cpf,
        ...
        tel_celular, email, nome_pai,nome_mae,
        nome_conjuge, datanascimento_conjuge,
        cargo_atual, databatismo, igrejabatismo,
        entradaPIB, formaentrada)

def autopreencher(evento,lista_nomes):
        nome_inserido = lista_nomes.value.strip()  # Obter o valor do campo
        if (CadastroPIB["Nome Completo"] == nome_inserido).any():
            indice = CadastroPIB[CadastroPIB["Nome Completo"] == nome_inserido].index[0]  # Obter o primeiro índice correspondente
